            ...
            actions={[
              {
                ...
                onClick: () => {
                  const first = recentReports.find((r) => selectedIds.has(r.id));
                  if (first) {
                    setToast({
                      message: `Downloading ${first.reportName}.${first.format.toLowerCase()}`,
                      variant: "info",
                    });
                  }
                },
            ...
            <thead>
              <tr className="border-b border-slate-200 bg-slate-50 text-[10px] uppercase tracking-wider text-slate-500 font-bold sticky top-0 z-10">
                <th className="w-10 px-3.5 py-3">
                  <input
                    type="checkbox"
                    checked={recentReports.length > 0 && recentReports.every((r) => selectedIds.has(r.id))}
                    onChange={() => {
                      const allIds = recentReports.map((r) => r.id);
                ...
                >
                  <td className="px-3.5 py-3">
                    <input
                      ...
                      className="rounded border-slate-300"
                      aria-label={`Select ${r.reportName}`}
                    />
                  </td>
            ...
            actions={[
              {
                ...
                onClick: () => {
                  const first = scheduledReports.find((sch) => selectedIds.has(sch.id));
                  if (first) {
                    setToast({
                      message: `Triggered manual execution for ${first.reportName}`,
                      variant: "info",
                    });
                  }
                },
            ...
            <thead>
              <tr className="border-b border-slate-200 bg-slate-50 text-[10px] uppercase tracking-wider text-slate-500 font-bold sticky top-0 z-10">
                <th className="w-10 px-3.5 py-3">
                  <input
                    type="checkbox"
                    checked={scheduledReports.length > 0 && scheduledReports.every((sch) => selectedIds.has(sch.id))}
                    onChange={() => {
                      const allIds = scheduledReports.map((sch) => sch.id);
                      const allSelected = allIds.every((id) => selectedIds.has(id));
                      setSelectedIds(allSelected ? new Set() : new Set(allIds));
                    }}
                    className="rounded border-slate-300"
            ...
            <tbody className="divide-y divide-slate-100 font-semibold text-slate-700">
              {scheduledReports.map((sch) => (
                <tr
                  ...
                  className={cn(
                    "hover:bg-slate-50/60 transition-colors",
                    selectedIds.has(sch.id) && "bg-emerald-50/40",
                  )}
                >
                  <td className="px-3.5 py-3">
                    <input
                      ...
                      onChange={() => {
                        const next = new Set(selectedIds);
                        if (next.has(sch.id)) next.delete(sch.id);
                        else next.add(sch.id);
                        setSelectedIds(next);
                      }}
                      className="rounded border-slate-300"
                      aria-label={`Select ${sch.reportName}`}
                    />
                  </td>
