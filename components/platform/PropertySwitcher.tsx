...
export function PropertySwitcher({ className }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (
        ...
        target.closest("[data-property-switcher-menu]")
      ) {
        return;
      }
      setOpen(false);
  ...
  const menuPanel = open ? (
    <div
      ...
      className={cn(
        "z-[100] overflow-hidden rounded-xl border border-slate-200 bg-white shadow-xl",
        isMobile
          ? "fixed left-3 right-3 max-h-[min(70dvh,22rem)]"
          : "absolute right-0 mt-1.5 w-72 max-h-56",
      )}
    >
      ...
      <div className="max-h-[min(50dvh,14rem)] overflow-y-auto py-1 sm:max-h-56">
        {loading ? (
  ...
  return (
    <div ref={containerRef} className={cn("relative", className)}>
      <button
        ...
        className={cn(
          "flex max-w-[8.5rem] items-center gap-1.5 rounded-lg border border-white/10 px-2 py-1.5 text-left transition-colors sm:max-w-[220px] sm:gap-2 sm:px-2.5 md:max-w-[260px]",
          "hover:bg-white/10",
          open && "bg-white/10",
        )}
      >
