...
export function LeaveManagementView() {
  ...
  // Apply Form State
  const [applyEmpId, setApplyEmpId] = useState("EMP-0101");
  const [applyLeaveTypeId, setApplyLeaveTypeId] = useState("lt-cl");
  ...
  const [applyFromDate, setApplyFromDate] = useState("2026-08-10");
  const [applyToDate, setApplyToDate] = useState("2026-08-12");
        ...
        <form onSubmit={handleSaveApplyLeave} className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              >
                {INITIAL_LEAVE_APPLICATIONS.map((staff) => (
                  <option key={staff.employeeId} value={staff.employeeId}>
                    👤 {staff.employeeName} ({staff.employeeId})
                  </option>
                ))}
              </select>
            </div>
