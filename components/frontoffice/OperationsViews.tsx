...
export function EarlyLateCheckView() {
  type EarlyLateRequest = {
    id: string;
    guest: string;
    room: string;
    type: "Early Check-in" | "Late Check-out";
    charges: number;
    approvedBy: string;
    reason: string;
    status: "Approved";
  ...
  const [sortBy, setSortBy] = useState("newest");
  const [guestSearch, setGuestSearch] = useState("");
  const [guest, setGuest] = useState<(typeof inHouseGuests)[0] | null>(null);
  const [requestType, setRequestType] = useState<"Early Check-in" | "Late Check-out">(
    "Early Check-in",
  );
  ...
  const [previewRequest, setPreviewRequest] = useState<EarlyLateRequest | null>(null);
  const [requests, setRequests] = useState<EarlyLateRequest[]>([
    {
      ...
      type: "Late Check-out",
      charges: 1500,
      approvedBy: "Front Manager",
      reason: "Flight at 8 PM — guest requested late checkout",
      status: "Approved",
      date: "23 Jun 2026",
    },
  ]);
  ...
  const suggestedCharge = requestType === "Early Check-in" ? 2000 : 1500;
  ...
  const filtered = useMemo(() => {
    ...
    let rows = requests.filter(
      (r) =>
        (typeFilter === "all" ||
          (typeFilter === "early" && r.type === "Early Check-in") ||
          (typeFilter === "late" && r.type === "Late Check-out")) &&
        (r.guest.toLowerCase().includes(q) ||
          r.type.toLowerCase().includes(q) ||
          r.id.toLowerCase().includes(q) ||
          r.approvedBy.toLowerCase().includes(q)),
    );
    if (sortBy === "charges-desc") rows = [...rows].sort((a, b) => b.charges - a.charges);
    if (sortBy === "guest") rows = [...rows].sort((a, b) => a.guest.localeCompare(b.guest));
    return rows;
  }, [requests, listSearch, typeFilter, sortBy]);

  const stats = useMemo(
    () => ({
      early: requests.filter((r) => r.type === "Early Check-in").length,
      late: requests.filter((r) => r.type === "Late Check-out").length,
      totalCharges: requests.reduce((s, r) => s + r.charges, 0),
    }),
    [requests],
  );
  ...
  const handleSubmit = () => {
    ...
    const charge = parseFloat(charges) || suggestedCharge;
    const record: EarlyLateRequest = {
      id: `EL-${String(requests.length + 1).padStart(2, "0")}`,
      guest: guest.guestName,
      room: guest.room,
      type: requestType,
      charges: charge,
      approvedBy: approvedBy || "Front Manager",
    ...
    resetForm();
    setToast(
      `${requestType} approved for ${guest.guestName} — ${formatINR(charge)} posted.`,
    );
  };
    ...
    <div className="space-y-4">
      <FormField label="Request Type">
        <div className="flex rounded-xl bg-slate-100/80 p-1">
          {(["Early Check-in", "Late Check-out"] as const).map((t) => (
            <button
              key={t}
              type="button"
              onClick={() => {
                setRequestType(t);
                setCharges(String(t === "Early Check-in" ? 2000 : 1500));
              }}
              className={cn(
      ...
      {guest && (
        <div className="rounded-lg bg-slate-50 px-3 py-2 text-xs text-slate-600">
          Room {guest.room} · Check-in {guest.checkIn} · Check-out {guest.checkOut}
        </div>
      )}
      ...
      <FormField label="Charges (₹)">
        <TextInput
          type="number"
          placeholder={String(suggestedCharge)}
          value={charges}
  ...
  return (
    <div className="space-y-5">
      ...
      <FOPageHeader
        ...
        description="Manage early arrival and late departure with manager approval."
        action={
          <Button
            size="sm"
            className="bg-blue-600 hover:bg-blue-700"
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-3">
        <StatMiniCard label="Early Check-ins" value={stats.early} accent="#2563eb" icon={LogIn} />
        <StatMiniCard label="Late Check-outs" value={stats.late} accent="#f59e0b" icon={LogOut} />
        <StatMiniCard label="Total Charges" value={formatINR(stats.totalCharges)} icon={IndianRupee} />
      </div>

      <FOSearchToolbar
        search={listSearch}
        onSearchChange={setListSearch}
        searchPlaceholder="Search guest, type, or request ID…"
        filterPills={{
          active: typeFilter,
          onChange: setTypeFilter,
          options: [
            { id: "all", label: "All" },
            { id: "early", label: "Early Check-in" },
            { id: "late", label: "Late Check-out" },
          ],
        }}
        hasActiveAdvancedFilters={sortBy !== "newest"}
        onClearAdvancedFilters={() => setSortBy("newest")}
      ...
      />
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-4 sm:p-5">
          <h2 className="mb-4 text-sm font-semibold text-slate-900">Recent Approvals</h2>
              ...
              >
                <div className="flex items-start justify-between gap-2">
                  <div>
                    ...
                    <p className="text-xs text-slate-500">{r.type} · Room {r.room}</p>
                  </div>
                  <span className="rounded-full bg-emerald-50 px-2 py-0.5 text-[11px] font-medium text-emerald-700">
                    {r.status}
                  </span>
            ...
            <table className="w-full text-left text-sm">
              <thead>
                <tr className="border-b border-slate-100 text-xs font-medium uppercase tracking-wide text-slate-500">
                  ...
                  <th className="pb-3 pr-4">Charges</th>
              ...
              <tbody>
                {filtered.map((r) => (
                  ...
                  >
                    ...
                    <td className="py-3.5 pr-4 font-semibold">{formatINR(r.charges)}</td>
                    <td className="py-3.5 pr-4">{r.approvedBy}</td>
                    <td className="py-3.5 pr-4">{r.date}</td>
                    <td className="py-3.5 pr-4">
                      <span className="rounded-full bg-emerald-50 px-2 py-0.5 text-xs font-medium text-emerald-700">
                        {r.status}
      ...
      <Drawer
        ...
        title="New Request"
        description="Approve early check-in or late check-out with charges."
        width="md"
        footer={
          <>
      ...
      <Drawer
        ...
        title={previewRequest?.guest ?? ""}
        description={previewRequest?.type}
        width="md"
        footer={
          previewRequest && (
            <Button variant="outline" onClick={() => setPreviewRequest(null)}>
              Close
            </Button>
          )
        }
      >
        {previewRequest && (
          <div className="space-y-5">
            <div className="flex items-center gap-3">
              <div
                className={cn(
                  ...
                  previewRequest.type === "Early Check-in"
              ...
              <div>
                <p className="text-2xl font-bold text-slate-900">
                  {formatINR(previewRequest.charges)}
                </p>
                <span
                  className={cn(
            ...
            <dl className="grid grid-cols-1 gap-3 text-sm">
              {[
                { icon: User, label: "Guest", value: previewRequest.guest },
                { icon: BedDouble, label: "Room", value: previewRequest.room },
                { icon: Clock, label: "Request ID", value: previewRequest.id },
                { icon: User, label: "Approved By", value: previewRequest.approvedBy },
                { icon: Calendar, label: "Date", value: previewRequest.date },
                { icon: FileText, label: "Reason", value: previewRequest.reason },
              ].map(({ icon: Icon, label, value }) => (
                <div key={label} className="flex gap-3 rounded-lg border border-slate-100 p-3">
                  <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-slate-100 text-slate-500">
                    <Icon className="h-3.5 w-3.5" />
