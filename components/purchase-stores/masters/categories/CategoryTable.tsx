...
import { Edit, Trash2, Layers, ArrowUpDown, ChevronLeft, ChevronRight, Package } from "lucide-react";
import { cn } from "@/lib/utils";
import type { CategoryItem } from "@/app/data/purchaseStoresMastersData";
...
export function CategoryTable({ categories, onEditCategory, onDeleteCategory, onResetFilters }: CategoryTableProps) {
  const [sortField, setSortField] = useState<keyof CategoryItem>("categoryCode");
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 gap-3 md:hidden">
        {paginatedCategories.map((cat) => (
          <div key={cat.id} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-2xs">
            <div className="flex items-start justify-between gap-2">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <p className="text-[11px] font-semibold text-emerald-700">{cat.categoryCode}</p>
                </div>
              </div>
              <span
                className={cn(
            ...
            <div className="mt-3 flex items-center justify-between border-t border-slate-100 pt-2.5">
              <span className="text-[11px] text-slate-500 truncate max-w-[200px]">{cat.description || "No description"}</span>
              <div className="flex items-center gap-1">
                <button type="button" onClick={() => onEditCategory(cat)} className="rounded-lg p-1.5 text-slate-600 hover:bg-slate-100 hover:text-amber-600" title="Edit Category">
                  <Edit className="h-4 w-4" />
                </button>
                <button type="button" onClick={() => onDeleteCategory(cat)} className="rounded-lg p-1.5 text-slate-600 hover:bg-red-50 hover:text-red-600" title="Delete Category">
            ...
            <thead className="sticky top-0 z-10 bg-slate-50/90 backdrop-blur-xs border-b border-slate-200 text-[11px] font-bold uppercase tracking-wider text-slate-500">
              <tr>
                <th className="py-3.5 px-4">
                  <button type="button" onClick={() => handleSort("categoryCode")} className="flex items-center gap-1 hover:text-slate-900">
                    Category Code <ArrowUpDown className="h-3 w-3" />
                  </button>
                </th>
                <th className="py-3.5 px-4">
                  <button type="button" onClick={() => handleSort("categoryName")} className="flex items-center gap-1 hover:text-slate-900">
            ...
            <tbody className="divide-y divide-slate-100 text-xs font-medium text-slate-700">
              {paginatedCategories.map((cat) => (
                <tr key={cat.id} className="transition-colors hover:bg-slate-50/80">
                  ...
                  <td className="py-3.5 px-4 text-slate-600 max-w-[220px] truncate">{cat.description || "—"}</td>
                  <td className="py-3.5 px-4 text-center whitespace-nowrap">
                    <span
                      className={cn(
                        "inline-flex rounded-full px-2.5 py-0.5 text-[11px] font-semibold",
                        cat.status === "Active" ? "bg-emerald-50 text-emerald-700 ring-1 ring-inset ring-emerald-200" : "bg-slate-100 text-slate-600 ring-1 ring-inset ring-slate-200"
                      )}
                    >
                      {cat.status}
                    </span>
                  </td>
                  <td className="py-3.5 px-4 text-center text-slate-500 text-[11px] whitespace-nowrap">{cat.createdDate}</td>
                  <td className="py-3.5 px-4 text-right whitespace-nowrap">
                    <div className="flex items-center justify-end gap-1">
                      <button type="button" onClick={() => onEditCategory(cat)} className="rounded-lg p-1.5 text-slate-500 hover:bg-amber-50 hover:text-amber-600" title="Edit Category">
                        <Edit className="h-4 w-4" />
                      </button>
                      <button type="button" onClick={() => onDeleteCategory(cat)} className="rounded-lg p-1.5 text-slate-500 hover:bg-red-50 hover:text-red-600" title="Delete Category">
                        <Trash2 className="h-4 w-4" />
                      </button>
                    </div>
                  </td>
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between px-1 py-2 text-xs text-slate-500">
        <div>
          Showing <span className="font-semibold text-slate-700">{Math.min((currentPage - 1) * itemsPerPage + 1, categories.length)}</span> to <span className="font-semibold text-slate-700">{Math.min(currentPage * itemsPerPage, categories.length)}</span> of <span className="font-semibold text-slate-700">{categories.length}</span> categories
        </div>
