...
export function AccountsToolsView() {
  ...
  return (
    ...
    >
      {/* Top Target Company & System Health Bar */}
      <div className="mb-4 rounded-2xl border border-slate-200 bg-white p-3.5 shadow-2xs space-y-3">
        ...
        <div className="flex items-center gap-1.5 overflow-x-auto pt-2 border-t border-slate-100">
          {[
            { id: "All", label: "All Maintenance Utilities", icon: Layers },
            { id: "Maintenance", label: "Ledger & Voucher Maintenance", icon: RotateCcw },
            { id: "Audit", label: "Database Audit & Parity", icon: ShieldCheck },
            { id: "DataManagement", label: "Bulk Data Import / Export", icon: Database },
            { id: "Security", label: "Financial Period Locking", icon: Lock },
          ].map((cat) => {
            const Icon = cat.icon;
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
        <StatMiniCard
          label="Active Utility Tools"
          value={`${toolsList.length} Utilities`}
          icon={Wrench}
        />
        <StatMiniCard
          label="Last Ledger Re-Index"
          value="Today 08:30 AM"
          sublabel="100% Parity Matched"
          icon={RotateCcw}
        />
        <StatMiniCard
          label="Database Integrity"
          value="0 Errors"
          sublabel="All Entries Balanced"
          icon={ShieldCheck}
        />
        <StatMiniCard
          label="Period Lock Status"
          value="Locked"
          sublabel="Locked thru 30/06/2026"
          icon={Lock}
        />
      </div>
